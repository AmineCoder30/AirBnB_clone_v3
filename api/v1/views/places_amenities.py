...
if getenv('HBNB_TYPE_STORAGE', 'fs') != 'db':
    ...
    def view_amenities_in_place(place_id):
        """Example endpoint returning a list of all amenities"""
    ...
    def create_amenity_in_place(place_id=None, amenity_id=None):
        """creates a link between a amenity and a place"""
        place = storage.get("Place", place_id)
        if place is None:
            abort(404)
    ...
    @app_views.route('/places/<place_id>/amenities/<amenity_id>',
                     methods=['POST'], strict_slashes=False)
    def create_amenity_in_place(place_id=None, amenity_id=None):
        """Example endpoint creates a link
        between a amenity and a place"""
        place = storage.get("Place", place_id)
        if place is None:
            abort(404)
        amenity = storage.get("Amenity", amenity_id)
